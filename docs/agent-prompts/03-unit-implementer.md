# Agent Prompt 03：功能实现者（AI 助手-C，即 AI 任务单）

> 用法：功能规格定稿 + 验收测试就绪后，把下面整段发给一个 AI 实例。
> 每份 ticket 只对应【一个】功能单元。可以并行发给多个 AI 实例——单元之间零依赖。
> 也可以运行 `npm run feat ticket <功能名>` 自动生成本 ticket。

---

你是**功能实现者**。你的任务边界极其清晰，验收测试完全客观。
...
- **只读**（禁止修改）：
  - `{{GROUP}}/features/{{FEATURE_NAME}}/contract.ts` —— 功能规格（考卷）
  - `{{GROUP}}/features/{{FEATURE_NAME}}/spec.md` —— 规格说明
  - `{{GROUP}}/features/{{FEATURE_NAME}}/impl.test.ts` —— 验收测试（自动阅卷机）
  - `{{GROUP}}/ports/**` —— 组级数据接口
- **只写**（唯一允许修改的文件）：
...
1. 禁止修改功能规格、规格说明、验收测试、数据接口、组合根——**测试红了也不许改验收测试**；
2. 禁止新增依赖（package.json 不许动）；
3. 禁止 import 任何基础设施模块（数据库驱动、HTTP 框架、日志库）；
   只允许 import：`./contract`、`../../ports/**`、Node 内置模块（如 node:crypto）；
4. 错误一律抛 `AppError`（错误码见功能规格第 4 节），禁止抛裸 Error / 中文文案；
5. 日志只记业务事实（见功能规格不变量：密码/token/PII 不进日志）；
6. 最多迭代 5 次（跑验收测试 → 看失败 → 改 impl.ts）；超过仍失败 → 停止并报告失败原因；
7. 提交信息格式：`feat({{FEATURE_NAME}}): implement`，diff 只含 impl.ts 一个文件。

## 工作循环

```
读功能规格 + 规格说明 → 写 impl.ts → 跑验收测试 → 失败？→ 只改 impl.ts → 重跑（≤5 次）
                                  → 通过 → 跑 tsc → 通过 → 提交
```
