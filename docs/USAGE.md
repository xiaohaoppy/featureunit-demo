...
> **环境要求**（自行安装）：Node.js ≥ 20 + npm、**git ≥ 2.x**——git 是框架硬依赖（定稿/预演/回滚），启动时自动检测。
...
| **🧪 业务测试** | 冒烟操作**动态发现**（健康检查 + 每个已接入功能，无需改前端）；**业务系统下拉**切换组；token 路由需会话 cookie（自动提示）；**存储模式自动跟随配置** |
...
- 「业务系统」下拉切换组（如 auth-service / order-service），cookie 自动流转；
- token 字段的功能路由从 cookie 取会话——无 cookie 会提示并返回 `INVALID_SESSION`（这是正确拦截）。
...
| 管理台需要另起业务服务吗？ | 试玩不需要——业务测试面板内置实例，存储模式自动跟随配置；真实访问/联调用 `npm run dev`，生产用 `npm start` |
