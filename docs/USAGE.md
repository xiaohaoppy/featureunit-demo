...
| **🔌 数据接口** | 接口清单（一句话/依赖功能/实现实现）；详情；📝 编辑（git 留痕）；＋ 手动新建 / 🤖 AI 生成（AI 助手-D）→ 7 项纪律初审 → 确认定稿 |
| **🧪 业务测试** | 冒烟（注册/登录/查我/登出/改密/改邮箱/找回密码），cookie 自动流转；**存储模式自动跟随配置** |
...
| ② 数据接口生成 | AI 助手-D 生成数据接口草稿 | 7 项纪律初审 | 确认 → 定稿 |
| ③ 功能规格生成 | AI 助手-A 生成功能规格草稿 | 结构/数据接口引用/tsc 初审 | 确认 → 定稿 |
| ④ 验收测试生成 | AI 助手-B 生成验收测试骨架 | 占位验收测试禁止定稿 | **补全断言** → 确认定稿 |
| ⑤ 实现 | AI 助手-C 自动迭代 | 验收测试全绿才提交；超限停手 | 确认（含接受"停手求援"） |
| ⑥ 接入接入 | AI 助手-E 生成接入 | **编译预检**（编译不过禁止确认） | 确认落盘 |
| ⑦ 完成 | — | 总闸 | 冒烟验证 |
...
注意：一键接入的锚点目前面向 auth-service——**新组第一个功能请人工接入**（或扩展锚点）。
...
| 新组第一个功能接入？ | 一键接入锚点面向 auth-service，新组请人工接入（编译预检会拦住错误接入） |
