...
组合根 `index.ts` + `adapters/http.ts` + `manifest.json` 由人接线。
**管理台"🔌 接线检查"会列出缺口清单**（import / AuthApi / 路由 / manifest 6 项），
人照着补；补完 `npm run check` 总闸全绿 → 冒烟 → 上线。
...
| 单元详情 | 4 文件查看；运行单单元判据；**编辑当前文件**（git 留痕）；🔌 接线检查 | 评审 / 验收 / 人工修改 |
