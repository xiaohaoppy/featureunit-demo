...
npm run admin        # 管理台：http://localhost:3001/admin（界面化操作，推荐）
```
...
| 命令 | 作用 |
|---|---|
| `new <名字>` | 生成功能单元 4 文件模板（管理台左侧"＋ 新功能单元"同款） |
| `ai-contract <名字> "<需求>"` | AI 生成契约 → 机器初审 → 人确认 → 冻结（管理台「AI 契约生成」同款） |
| `test <名字>` | 只跑该单元的判据（AI 迭代用） |
| `ticket <名字>` | 打印该单元的 AI 任务单（管理台「Ticket」同款） |
| `check` | 总闸 |

> 本教程以 CLI 为例演示；**管理台里每个命令都有对应按钮**（详见 `USAGE.md`），
> 界面与 CLI 共用同一套逻辑，结果完全一致。
...
```bash
# 真实模式（需要 API Key：管理台「配置」tab 填写，或环境变量）
...
也可以全程在管理台「AI 契约生成」面板操作：填功能名 + 需求 + 模式 →
生成草稿 → 看机器初审清单 → **10 项逐条点"通过/打回"** → 全部通过才可冻结。
（CLI 与管理台共用同一套逻辑，行为一致。）
...
**管理台「单元详情」的"🔌 接线检查"会列出 6 项缺口清单，人照着补：**
...
功能上线后，日常开发就是循环（**CLI 与管理台二选一，结果一致**）：

```bash
npm run feat -- new <新功能>                          # 开新单元（或管理台"＋ 新功能单元"）
npm run feat -- ai-contract <新功能> "<需求>"          # AI 生成契约 → 你确认 → 冻结（或管理台 AI 面板）
# 写判据（impl.test.ts）→ 红灯确认
npm run feat -- ticket <新功能>                        # 打印任务单（或管理台 Ticket 面板复制）
# 把任务单发给 AI → AI 只写 impl.ts → 判据全绿
npm run feat -- check                                 # 总闸（或管理台"运行全部判据"）
```

并行技巧：单元之间零依赖（只共享冻结的端口），**多个 ticket 可以并行发给
多个 AI 实例**，互不阻塞。组合根始终由人接（接线检查器会列出缺口）。
...
7. **配置写哪都一样，来源要能看出来**——管理台「配置」面板标注每个值
   "本地配置/环境变量/默认值"来源，改错了能一眼定位；
8. **界面不替代纪律**——管理台能编辑冻结区文件、能跑判据，但冻结、接线检查、
   git 留痕这些"人的动作"一步都不会少，只是从终端搬进了界面。
